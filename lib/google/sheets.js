...
export async function getSheetValues(tabName, { fresh = false } = {}) {
  const token = await getGoogleAccessToken();
  const spreadsheetId = encodeURIComponent(process.env.GOOGLE_SHEETS_SPREADSHEET_ID);
  const range = encodeURIComponent(`${tabName}!A:Z`);
  const response = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}`, fresh ? { headers: { authorization: `Bearer ${token}` }, cache: 'no-store' } : { headers: { authorization: `Bearer ${token}` }, next: { revalidate: 180, tags: ['joybundle-catalog'] } });
  if (!response.ok) {
