...
export async function createOrder(input, dependencies = {}) {
  ...
  return { order_number: orderNumber, customer_name: customer.customer_name, total_paise: String(subtotal), payment_status: 'PENDING', party_date: customer.party_date, area: customer.area, pin_code: customer.pin_code, items: serverItems.map(({ product, quantity, lineTotal }) => ({ product_code: product.product_code, name: product.name, quantity, line_total_paise: String(lineTotal) })) };
}
